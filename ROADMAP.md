...
- [x] Presigned URL uploads — browser uploads directly to R2 via presigned PUT URL, zero server RAM. Three-step flow: `POST /presign` → `PUT` to R2 → `POST /confirm`. Fixes 502 Bad Gateway on large files (OOM from server buffering)
- [x] Upload progress bar — `XMLHttpRequest.upload.onprogress` on the direct R2 upload + animated progress bar in UploadPage
