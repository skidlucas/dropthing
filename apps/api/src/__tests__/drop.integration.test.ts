...
afterAll(async () => {
  await Effect.runPromise(
    Effect.scoped(
      ...
      }).pipe(Effect.provide(DrizzleService.layer))
    )
  );
});
...
describe('DropService integration — zero-knowledge proof', () => {
  ...
  it.live('encrypted text drop: stored content in DB is NOT the original plaintext', () =>
    Effect.gen(function* () {
      ...
      const [row] = yield* db.select().from(dropsTable).where(eq(dropsTable.id, drop.id));
